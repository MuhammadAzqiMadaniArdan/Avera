...
export default function CallbackPage() {
  ...
  useEffect(() => {
    ...
    if (!codeVerifier) {
      ...
      setTimeout(
        () => router.replace("/auth/login?error=missing_verifier"),
        2000,
      );
      return;
    }
    ...
    const handleExchangeToken = async (code: string, verifier: string) => {
      try {
        ...
        const redirectUri = `${process.env.NEXT_PUBLIC_BASE_URL}/api/v1/auth/callback`;
        ...
        setTimeout(
          () => router.replace("/auth/login?error=token_exchange_failed"),
          2000,
        );
      }
    };
